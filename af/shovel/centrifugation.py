...
def httpt_body(response):
    ...
    if body[-7:] == "\r\n0\r\n\r\n":  # looks like chunked
        ...
        for k, v in response["headers"].iteritems():
            ...
            raise RuntimeError(
                "Chunked body without `Transfer-Encoding: chunked`", response["headers"]
            )
        out = []
        offset = body.index("\r")
...
def load_autoclaved_db_todo(pgconn, files, bucket):
    ...
    with pgconn.cursor() as c:
        c.execute(
            "SELECT filename, file_sha1, code_ver FROM autoclaved WHERE bucket_date = %s",
            (bucket,),
        )
        for filename, file_sha1, code_ver in c:
            ...
            if filename not in files:
                raise RuntimeError(
                    "Unknown filename in autoclaved PG table present in autoclaved/index.json.gz",
                    filename,
                )
            elif (
                files[filename] != file_sha1
            ):  # autoclaved file changed, e.g. PII cleanup
                reingest.add(filename)
            elif (
                code_ver < CODE_VER
            ):  # including CODE_VER_REPROCESS that is less than any CODE_VER
...
def copy_meta_from_index(pgconn, ingest, reingest, autoclaved_index, bucket):
    ...
    autoclaved_xref_seq = itertools.count(
        1000000000
    )  # actual value does not matter, that's just DBG
    ...
    dest_wbuf = {
        ("/file", "new"): PGCopyFrom(
            pgconn, "autoclaved_meta", columns=autoclaved_columns[1:]
        ),
        ("/file", "re"): PGCopyFrom(
            pgconn, "autoclaved_meta", columns=autoclaved_columns
        ),
        ("/report", "new"): PGCopyFrom(
            pgconn, "report_meta", columns=report_columns[1:]
        ),
        ("/report", "re"): PGCopyFrom(pgconn, "report_meta", columns=report_columns),
        ("datum", "new"): PGCopyFrom(
            pgconn, "measurement_meta", columns=measurement_columns[1:]
        ),
        ("datum", "re"): PGCopyFrom(
            pgconn, "measurement_meta", columns=measurement_columns
        ),
        "badblob": PGCopyFrom(
            pgconn,
            "badblob_meta",
    ...
    with gzip.GzipFile(autoclaved_index, "r") as indexfd:
        ...
        autoclaved_xref, report_xref = (
            None,
            None,
        )  # act as "should-process" flag as well
        ...
        for _, doc in autoclaving.stream_json_blobs(indexfd):
            ...
            if t == "file":  # autoclaved file
                ...
                report_datum = 0
                if (
                    autoclaved_xref is not None
                    and rpt["textname"] not in DUPLICATE_REPORTS
                ):
                ...
                if autoclaved_xref is not None and report_xref is not None:
                    ...
                    write_row(
                        ...
                        "0\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\t{}\n".format(
                            ...
                            - frm[
                                "text_off"
                            ],  # intra-frame off, as text_off is absolute
                            doc[
                                "text_size"
                            ],  # intra-frame size equals to stream size :)
                ...
                raise RuntimeError(
                    "Unknown index.json.gz row type", autoclaved_index, doc
                )
    ...
    with pgconn.cursor() as c:
        c.execute(
            """UPDATE autoclaved_meta AS meta SET autoclaved_no = atclv.autoclaved_no
        ...
        c.execute(
            """UPDATE report_meta AS meta SET autoclaved_no = atclv.autoclaved_no
            FROM autoclaved_meta AS atclv
            WHERE atclv.autoclaved_xref = meta.autoclaved_xref
        """
        )  # every row
        c.execute(
            """UPDATE report_meta AS meta SET report_no = rpt.report_no
            FROM report AS rpt
        ...
        c.execute(
            """UPDATE measurement_meta AS meta SET report_no = rpt.report_no
        ...
        c.execute(
            """UPDATE measurement_meta AS meta SET msm_no = msm.msm_no
            FROM (
                SELECT
                    msm_no,
                    report_no,
        ...
        if stat != {"atclv": 0, "rpt": 0, "msm": 0}:
            raise RuntimeError(
                "Unable to preserve rows properly while re-ingesting", bucket, stat
            )
...
def delete_data_to_reprocess(pgconn, bucket):
    ...
    with pgconn.cursor() as c:
        ...
        c.execute(
            "SELECT DISTINCT code_ver FROM autoclaved WHERE bucket_date = %s", [bucket]
        )
        ...
        for ver, next_ver in zip(code_ver, code_ver[1:] + [CODE_VER]):
            if (
                0 <= ver < CODE_VER
            ):  # NULL is already at `del_msm`, CODE_VER never requires cleanup
                # `del_msm` is updated to include measurements for `reprocess` action.
                c.execute(
                    """INSERT INTO del_msm SELECT msm_no
                    FROM autoclaved
            ...
            for tbl in DATA_TABLES:
                ...
                if should_be_cleaned and should_be_cleaned_now:
                    if (
                        "body_simhash" in tbl.columns
                        and "body_text_simhash" in tbl.columns
                    ):
                        rowcount = None
                        c.execute(
                            """WITH del AS (DELETE FROM {} WHERE msm_no IN (SELECT msm_no FROM del_msm)
                        ...
                        for row in c:
                            if row[0] is not None:
                                key = str(
                                    row[0]
                                )  # convert memoryview(Py3) / buffer(Py2) to str / bytes
                                assert len(key) == 256 / 8
                                value = BaseHttpFeeder.simhash_value_type.pack(
                                    row[1], row[2]
                                )
                        ...
                        c.execute(
                            "DELETE FROM {} WHERE msm_no IN (SELECT msm_no FROM del_msm)".format(
                                tbl.data_table
                            )
                        )
                        rowcount = c.rowcount
                    print "DELETE FROM {} because of ver={:d} ~ {:d} rows gone".format(
                        tbl.data_table, ver, rowcount
                    )
...
def prepare_destination(pgconn, stconn, bucket_code_ver, bucket_date):
    ...
    badrow_sink = PGCopyFrom(
        pgconn, BadrowFeeder.sink_table, columns=BadrowFeeder.columns
    )
        ...
        for cls in DATA_TABLES:
            if ver < cls.min_compat_code_ver:  # what a useful table for this version!
                if cls not in data_tables:
                    feeder = cls(
                        *data_tables_args.get(cls, tuple())
                    )  # feels a bit like perl :-/
...
def copy_data_from_autoclaved(pgconn, stconn, in_root, bucket, bucket_code_ver):
    ...
    for code_ver, autoclaved_no, report_no, msm_no, datum in iter_autoclaved_datum(
        pgconn, in_root, bucket
    ):
        queue, exc = [], []
        if code_ver is None:
            try:
                ...
                badmeta.sink.write(
                    badmeta.badmeta_row(autoclaved_no, report_no, report=sys.exc_info())
                )
...
def iter_autoclaved_datum(pgconn, autoclaved_root, bucket):
    ...
    with pgconn.cursor("lz4meta") as cmeta:
        ...
        for (filename, file_size, file_crc32, file_sha1), itfile in groupby(
            cmeta, itemgetter(0, 1, 2, 3)
        ):
            ...
            with open(os.path.join(autoclaved_root, filename)) as fd:
                ...
                db_cksum = (
                    ...
                    file_crc32,
                    str(file_sha1),
                )  # file_sha1 is returned as buffer()
                fd_cksum = (fd.size, fd.crc32, fd.sha1)
                if db_cksum != fd_cksum:
                    raise RuntimeError(
                        "Checksum mismatch", filename, db_cksum, fd_cksum
                    )
...
class BaseFeeder(object):
    ...
    def init_sink(self, pgconn, badsink=None):
        ...
        self.sink = PGCopyFrom(
            pgconn,
            self.sink_table,
            columns=self.columns,
            badsink=badsink,
            badrow_fmt=badrow_fmt,
        )
        return self.sink
...
class MeasurementExceptionFeeder(BaseFeeder):
    ...
    def __init__(self, pgconn):
        self.pgconn = pgconn
        create_temp_table(
            pgconn, self.sink_table, "msm_no integer NOT NULL, exc integer[] NOT NULL"
        )
    ...
    def report_row(self, report_no, datum):
        if self.prev_no == report_no:
            for ndx, key in enumerate(self.common_keys):
                if (
                    self.prev_keys[ndx] != datum[key]
                ):  # also enforces existence of common_keys
                    raise RuntimeError(
                        "report key mismatch", key, self.prev_keys, datum[key]
                    )
            return ""
        else:
...
class BadrowFeeder(object):
    ...
    def badrow_row(table, datum):
        return "{}\t{:d}\t\\\\x{}\n".format(
            pg_quote(table), CODE_VER, datum.encode("hex")
        )
...
class TcpFeeder(BaseFeeder):
    ...
    def row(msm_no, datum):
        ...
        for el in test_keys.get("tcp_connect", ()):
            ...
            assert failure is not None or (
                el["status"]["success"] == True and el["status"]["blocked"] == False
            )
            if IPV4_RE.match(ip):  # domains?! why?! # TODO: is not IPv6-ready :-(
                ...
                if elctrl:
                    ...
                    assert isinstance(elctrl["status"], bool) and elctrl["status"] == (
                        elctrl["failure"] is None
                    )
                    ...
                    control_failure = (
                        None
                    )  # well, it's hard to state anything about `control_api_failure`
                ret += "{:d}\t{}\t{:d}\t{}\t{}\t{}\n".format(
                    msm_no,
                    ip,
                    port,
...
class DnsFeeder(BaseFeeder):
    ...
    def row(msm_no, datum):
        ...
        control_cname = none_if_len0(
            [_ for _ in addrs if _ not in control_ip]
        )  # O(n^2), but list is tiny
        ...
        for q in queries:
            if q["query_type"] == "A":  # almost always
                ...
                test_ip = none_if_len0(
                    sorted([a["ipv4"] for a in q["answers"] if a["answer_type"] == "A"])
                )
                test_cname = none_if_len0(
                    [a["hostname"] for a in q["answers"] if a["answer_type"] == "CNAME"]
                )
                ret += "{:d}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(
                    msm_no,
                    pg_quote(q["hostname"]),
...
class VanillaTorFeeder(BaseFeeder):
    ...
    def row(msm_no, datum):
        ...
        if success is None:
            if (
                t["tor_log"] is None
                and t["tor_progress_tag"] is None
                and t["tor_progress"] == 0
                and t["timeout"] > 60
                and t["error"] is None
                and t["tor_progress_summary"] is None
            ):
        ...
        if t.get("transport_name") not in (None, "vanilla"):
            raise RuntimeError(
                "`vanilla_tor` data with non-vanilla `transport_name`",
        ...
        return "{:d}\t{:d}\t{}\t{:d}\t{}\t{}\t{}\t{}\t{}\n".format(
            ...
            pg_quote(success),
            pg_quote(
                t.get("tor_progress_tag")
            ),  # XXX: looks like dup of `tor_progress`
            pg_quote(
                t.get("tor_progress_summary")
            ),  # XXX: looks like dup of `tor_progress`
            pg_quote(
                t.get("tor_version", "0.0.0")
            ),  # fav versions are "00:40:28.580", "22:09:16.876" and "None"
            pg_quote(tor_log),
        )
...
class BaseHttpFeeder(BaseFeeder):
    ...
    def simhash_cache_update(cls, pgconn, bucket_date):
        ...
        with pgconn.cursor() as c:
            ...
            WHERE msm_no IN (SELECT * FROM shcmsm) AND msm_no >= %s AND msm_no <= %s
                  AND body_sha256 IS NOT NULL
            """.format(
                    HttpRequestFeeder.data_table, HttpControlFeeder.data_table
                ),
                [msm_min, msm_max, msm_min, msm_max],
            )
            for row in c:
    ...
    def _pop_request(cls, datum, request):  # .test_keys.requests[].request element
        if (
            "url" in request and request["url"] == datum["input"]
        ):  # may be already popped
            del request["url"]
    ...
    def close(self):
        if self.simhash_cache:
            ...
            print "SimhashCache: len {:d}, hit {:d} ({:.1f} MiB), get {:d} ({:.1f} MiB), hit-rate {:.3f} ({:.3f} of bytes)".format(
                ...
                s["get"],
                s["getbyte"] / mb,
                s["hit"] * 1.0 / s["get"] if s["get"] else 0.0,
                s["hitbyte"] * 1.0 / s["getbyte"] if s["getbyte"] else 0.0,
            )
...
class HttpControlFeeder(BaseHttpFeeder):
        ...
        if datum["test_name"] == "web_connectivity":
            ...
            if r:
                ...
                ret = "{:d}\tFALSE\t{}\t{}\t{}\t{}\t{}\t\\N\t\\N\t\\N\n".format(
                    ...
                    pg_quote(ujson.dumps(http_headers(r["headers"]))),
                )  # seems, empty headers dict is still present it case of failure
        elif datum["test_name"] == "http_requests":
            for r in datum["test_keys"]["requests"]:
                if r["request"]["tor"]["is_tor"]:
                    response = r["response"]
                    body = httpt_body(response)
                    body_sha256, body_simhash, body_text_simhash = self.pg_body_hash(
                        body
                    )
                    if response is None:
                        response = {}
                    ...
                    ret += "{:d}\tTRUE\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(
                        ...
                        pg_quote(
                            len(body) if body is not None else r.get("response_length")
                        ),
    ...
    def pop(self, datum):
        if datum["test_name"] == "web_connectivity":
            ...
            for r in datum["test_keys"]["requests"]:
                if (
                    r["request"]["tor"].get("is_tor") == True
                ):  # may be popped by HttpRequestFeeder
                    response = r["response"]
...
class HttpRequestFeeder(BaseHttpFeeder):
    ...
    def _row(self, msm_no, r):
        ...
        if (
            response is None
        ):  # may be `null` both for web_connectivity and http_requests
            response = {}
        headers = response.get("headers")
        if headers is not None:
            headers = ujson.dumps(http_headers(headers))
    ...
    def pop(self, datum):
        if datum["test_name"] == "web_connectivity":
            ...
            for r in datum["test_keys"]["requests"]:
                if (
                    r["request"]["tor"].get("is_tor") == False
                ):  # may be popped by HttpControlFeeder
                    self._pop(datum, r)
...
class HttpVerdictFeeder(BaseFeeder):
    ...
    blndx = (
        columns.index("blocking") - 1
    )  # may be BOTH bolean and text, needs some type convertion
    ...
    def __init__(self, conn, table, value, key, maxsize=4194304):
        ...
        with self.__conn, conn.cursor() as c:
            ...
            if len(key) == 1:
                ...
                for _ in c:
                    self[_[1:]] = _[0]
        self.__select = "SELECT {} FROM {} WHERE ".format(value, table) + " AND ".join(
            _ + " = %s" for _ in key
        )
        self.__insert = "INSERT INTO {} ({}) VALUES({}) ON CONFLICT DO NOTHING RETURNING {}".format(
            table, ", ".join(key), ", ".join(["%s"] * len(key)), value
        )
        size = self.getsizeof()
        if size > maxsize:
            raise RuntimeError(
                "PostgresDict is huge, tune maxsize of refactor",
                size,
                maxsize,
                table,
                key,
    ...
    def getsizeof(self):
        if isinstance(next(self.iterkeys(), ()), tuple):
            return sys.getsizeof(self) + sum(
                sys.getsizeof(_k)
                + sys.getsizeof(_v)
                + sum(sys.getsizeof(_) for _ in _k)
                for _k, _v in self.iteritems()
            )
        else:
            return sys.getsizeof(self) + sum(
                sys.getsizeof(_k) + sys.getsizeof(_v) for _k, _v in self.iteritems()
            )
...
def meta_pg(in_root, bucket, postgres):
    ...
    assert (
        in_root[-1] != "/"
        and "/" not in bucket
        and os.path.isdir(os.path.join(in_root, bucket))
    )
    ...
    with pgconn, pgconn.cursor() as c:  # main transaction
        ...
        if not (ingest or reingest or reprocess):
            ...
            return
        print "bucket={}: ingest {:d}, reingest {:d}, reprocess {:d}".format(
            bucket, len(ingest), len(reingest), len(reprocess)
        )
        ...
        if c.rowcount != len(reprocess):
            raise RuntimeError(
                "Bad rowcount of reprocess files", c.rowcount, len(reprocess)
            )
        c.execute(
            """
        ...
        if c.rowcount != len(ingest) + len(reingest):
            raise RuntimeError(
                "Bad rowcount in autoclaved_meta",
                c.rowcount,
                len(ingest),
                len(reingest),
            )
